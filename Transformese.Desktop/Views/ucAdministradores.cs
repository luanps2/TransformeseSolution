...
{
    public class ucAdministradores : BaseCrudListControl
    {
        public ucAdministradores(System.Net.Http.HttpClient http) : base(http, "Administradores")
        {
            _ = LoadDataAsync();
        }
        ...
        private async Task LoadDataAsync()
        {
            ...
            // filtro simples: tipoUsuarioId == 1 (assumindo 1=Administrador)
            if (data != null && data.Value.ValueKind == JsonValueKind.Array)
            {
                var arr = new System.Text.Json.Nodes.JsonArray();
                foreach (var u in data.Value.EnumerateArray())
                {
                    if (u.TryGetProperty("tipoUsuarioId", out var t) && t.GetInt32() == 1)
        ...
        protected override async Task OnDeleteAsync()
        {
            if (_grid.CurrentRow == null) return;
            var id = int.Parse(_grid.CurrentRow.Cells[0].Value?.ToString() ?? "0");
            if (MessageBox.Show($"Excluir administrador #{id}?", "Confirmação", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
            {
