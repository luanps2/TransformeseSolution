...
{
    public class ucProfessores : BaseCrudListControl
    {
        public ucProfessores(System.Net.Http.HttpClient http) : base(http, "Professores")
        {
            _ = LoadDataAsync();
        }
        ...
        private async Task LoadDataAsync()
        {
            ...
            if (data != null && data.Value.ValueKind == JsonValueKind.Array)
            {
                ...
                {
                    if (u.TryGetProperty("tipoUsuarioId", out var t) && t.GetInt32() == 2) // assume 2=Professor
        ...
        protected override async Task OnDeleteAsync()
        {
            if (_grid.CurrentRow == null) return;
            var id = int.Parse(_grid.CurrentRow.Cells[0].Value?.ToString() ?? "0");
            if (MessageBox.Show($"Excluir professor #{id}?", "Confirmação", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
            {
